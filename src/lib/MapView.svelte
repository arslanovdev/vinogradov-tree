<script lang="ts">
  ...
  onMount(() => {
    ...
    for (const n of PLACE_NODES) {
      const cnt = (people[n.key] || []).length;
      bounds.extend([n.lon, n.lat]);
      const r = 30 + Math.min(cnt, 18) * 2.4;
      const wrap = document.createElement('div');
      wrap.style.cssText = `display:flex;flex-direction:column;align-items:center;cursor:${cnt ? 'pointer' : 'default'}`;
      const dot = document.createElement('div');
      dot.style.cssText = `width:${r}px;height:${r}px;border-radius:50%;background:${placeColor(n.kind)};opacity:${cnt ? 0.92 : 0.4};border:3px solid #fffdf9;box-shadow:0 3px 10px rgba(60,48,34,.3);display:flex;align-items:center;justify-content:center;color:#fffdf9;font:800 ${cnt > 9 ? 13 : 15}px Manrope,sans-serif`;
      dot.textContent = cnt ? String(cnt) : '';
      const lbl = document.createElement('div');
      lbl.style.cssText = 'margin-top:3px;font:700 11.5px Spectral,serif;color:#322c24;background:rgba(255,253,249,.82);padding:1px 6px;border-radius:6px;white-space:nowrap;box-shadow:0 1px 3px rgba(60,48,34,.12)';
      lbl.textContent = n.name + (n.kind === 'war' ? ' ✝' : '');
      wrap.appendChild(dot); wrap.appendChild(lbl);
      if (cnt) wrap.addEventListener('click', () => (place = n.key));
      new maplibregl.Marker({ element: wrap, anchor: 'center' }).setLngLat([n.lon, n.lat]).addTo(map);
    }
    const fit = () => { try { map.fitBounds(bounds, { padding: { top: 150, bottom: 80, left: 70, right: 70 }, maxZoom: 6.5, duration: 0 }); } catch { /* */ } };
    fit();
    setTimeout(() => { map.resize(); fit(); }, 120);

    map.on('load', () => {
      map.resize(); fit();
      const feats = PLACE_ARROWS.map((a) => {
        const f = PLACE_NODES.find((n) => n.key === a.from), t = PLACE_NODES.find((n) => n.key === a.to);
        if (!f || !t) return null;
        return { type: 'Feature' as const, properties: { kind: a.kind }, geometry: { type: 'LineString' as const, coordinates: [[f.lon, f.lat], [t.lon, t.lat]] } };
      }).filter(Boolean);
      map.addSource('arrows', { type: 'geojson', data: { type: 'FeatureCollection', features: feats as never } });
      map.addLayer({ id: 'mig', type: 'line', source: 'arrows', filter: ['==', ['get', 'kind'], 'mig'], layout: { 'line-cap': 'round' }, paint: { 'line-color': '#b0876a', 'line-width': 2.6, 'line-opacity': 0.8 } });
      map.addLayer({ id: 'war', type: 'line', source: 'arrows', filter: ['==', ['get', 'kind'], 'war'], paint: { 'line-color': '#a4553f', 'line-width': 1.8, 'line-opacity': 0.6, 'line-dasharray': [2, 2.5] } });
    });
    return () => map.remove();
  });
</script>
...
<div class="title">
  ...
  <div class="cap">Узлы — по местам рождения, жизни и гибели; размер = число людей. Линии — переселение рода, пунктир — фронтовые судьбы.</div>
</div>
...
<style>
  ...
  .title { position: absolute; left: 30px; top: 26px; pointer-events: none; z-index: 31; max-width: 320px; }
