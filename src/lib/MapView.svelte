<script lang="ts">
  ...
  import { mapPlaces, mapArrows, placeColor, placesMissingCoords, placeIntro } from '../model/places';
  ...
  const missing = $derived(placesMissingCoords(tree));
...
<style>
  ...
  .title { position: fixed; left: 30px; top: 26px; pointer-events: none; z-index: 31; max-width: 340px; }
  ...
  .panel.mobile { top: auto; bottom: 0; left: 0; right: 0; width: 100%; max-width: none; height: 62%; border-left: none; border-top: 1px solid #ece5da; border-radius: 22px 22px 0 0; }
  ...
  .pbio { font-size: 13px; line-height: 1.55; color: #4a443b; background: #fbf7ef; border: 1px solid #efe7d8; border-radius: 8px; padding: 10px 12px; margin: 10px 0; }
